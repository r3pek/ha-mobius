...
class MobiusSceneCard extends LitElement {
  ...
  static getStubConfig(hass, entities) {
    const guess = (entities || []).find(
      (e) => e.startsWith("select.") && e.includes("scene")
    );
    return { entity: guess || "" };
  }
  ...
  _renderTile(name, isActive) {
    const icon = name === NONE_OPTION ? "mdi:calendar-clock" : iconFor(name);
    const label = name === NONE_OPTION ? "Normal schedule" : name;
    return html`
      <button
  ...
  render() {
    ...
    const options = stateObj.attributes.options || [NONE_OPTION];
    const duration = stateObj.attributes.duration_remaining_seconds;
    const activeSceneName = current !== NONE_OPTION ? current : null;

    return html`
      <ha-card>
        <div class="header">
          <div class="title">${stateObj.attributes.friendly_name || "Scene"}</div>
        </div>
        ${activeSceneName
          ? html`
              <div class="status active">
                ...
                <div class="status-text">
                  <div class="status-main">${activeSceneName} active</div>
                  ${duration != null
                    ? html`<div class="status-sub">${formatDuration(duration)} remaining</div>`
                    : nothing}
                </div>
              </div>
            `
          : html`<div class="status">Running the normal schedule.</div>`}
        <div class="tiles">
          ${options.map((name) => this._renderTile(name, name === current))}
        </div>
      </ha-card>
    `;
  }
...
customElements.define("mobius-scene-card", MobiusSceneCard);
